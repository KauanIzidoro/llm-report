...
def storage_context_file(user_file: UploadFile):
    ...
    with open(
        os.path.join(SETTINGS.USER_FILE_PATH, user_file.filename), 'wb'
    ) as buffer:
...
def process_user_input(user_input: str) -> ModelInput:
    ...
    context = ''
    for file_name in os.listdir(SETTINGS.USER_FILE_PATH):
        file_path = os.path.join(SETTINGS.USER_FILE_PATH, file_name)
    ...
    validate_prompt = ModelInput(prompt=f'{user_input}\n\n{context}')
